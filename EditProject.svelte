<script>
    ...
    onMount(() => {
        // Get project_id and current_section from the URL or wherever you are storing them
        // For example, you can use the Svelte router or pass them as props
        project_id = "your_project_id";
        current_section = "your_current_section";
    });

    async function addStep() {
        const formData = new FormData();
        formData.append("step_description", step_description);
        formData.append("section", current_section);

        try {
        const response = await fetch(`/edit_project/${project_id}/add_step`, {
            method: "POST",
            body: formData,
        });

        if (response.ok) {
            // The step was added successfully
    ...
    onMount(() => {
        ...
        // For example, you can use the Svelte router or pass them as props
        project_id = "your_project_id";
        navigational_lights_steps = ["step1", "step2", "step3"]; // Replace with your data
    });

    async function deleteStep(section, step) {
        const formData = new FormData();
        formData.append("step_to_delete", step);
        formData.append("section", section);

        try {
        const response = await fetch(`/edit_project/${project_id}/delete_step`, {
            method: "POST",
            body: formData,
        });
...
<!--
    ...
    <div id="add-step-form-container" style="display: none;">
        <h2>Add Step:</h2>
        <form on:submit|preventDefault={addStep}><!--<form method="POST" action="/edit_project/{{ project_id }}/add_step">-->
            <label for="step_description">Step Description:</label>
            <input bind:value={step_description} type="text" id="step_description" name="step_description">
